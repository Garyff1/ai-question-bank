...
- 桌面端：Windows 测试版已开放下载，启动桌面程序时自动拉起内置 FastAPI 后端。
...
| Android v1.0.5 APK | [下载 APK](https://github.com/Garyff1/ai-question-bank/releases/download/android-v1.0.5/ai-question-bank-android-v1.0.5.apk) |
| Android v1.0.5 Release | [查看 Release](https://github.com/Garyff1/ai-question-bank/releases/tag/android-v1.0.5) |
| Desktop v0.1.0 Windows ZIP | [下载桌面版](https://github.com/Garyff1/ai-question-bank/releases/download/desktop-v0.1.0/ai-question-bank-desktop-windows.zip) |
| Desktop v0.1.0 Release | [查看 Release](https://github.com/Garyff1/ai-question-bank/releases/tag/desktop-v0.1.0) |
| 部署说明 | [DEPLOYMENT.md](DEPLOYMENT.md) |

Android v1.0.5 APK 校验值：

```text
SHA-256: CB59B634C9713AB951430212C6A3D974E5132387706740C00DB95605A5C593AF
```

Desktop v0.1.0 Windows ZIP 校验值：

```text
SHA-256: EF3596BE00EB0907D6382A000CF0496E607735FE61B74CD0BEE17A4AFB522E40
```
...
1. 在安卓手机或平板上下载 [ai-question-bank-android-v1.0.5.apk](https://github.com/Garyff1/ai-question-bank/releases/download/android-v1.0.5/ai-question-bank-android-v1.0.5.apk)。
2. 打开 APK 安装包。
...
- 官网短链接：[https://aichuti.ccwu.cc/download/desktop-windows.zip](https://aichuti.ccwu.cc/download/desktop-windows.zip)
- GitHub Release：[desktop-v0.1.0](https://github.com/Garyff1/ai-question-bank/releases/tag/desktop-v0.1.0)
...
| 小米 MiMo | `https://api.xiaomimimo.com/v1` | `mimo-v2.5-pro` | 接口路径兼容 `/chat/completions`，但鉴权 Header 使用 `api-key: <MIMO_API_KEY>`，不是普通的 `Authorization: Bearer`。 | [MiMo First API Call](https://mimo.mi.com/docs/en-US/quick-start/summary/first-api-call) |
...
官网首页的 `#apikey` 区块已经补充“API Key 获取与配置指南”，会把新用户引导到各服务商控制台创建 Key，再回到 App 或桌面版填写 Key、Base URL 和模型名称。
...
| P1 | 错题本没有“错题训练”入口。 | Android v1.0.5 已加入“错题抽卡”挑战，可随机抽取历史错题练习。 |
| P1 | 错题没有按资料划分，多个资料的错题混在一起。 | 增加“按资料收纳”的错题分组，每份资料维护独立错题集合，同时保留全部错题视图。 |
| P1 | 缺少学习激励系统。 | Android v1.0.5 已加入经验值、连续打卡、错题抽卡和限时三倍经验。 |
| P2 | 首次使用缺少引导页。 | 已加入首次启动引导，解释导入资料、生成题目、错题本和 API 配置入口。 |
| P2 | API Key 获取路径不清晰。 | 官网 `#apikey` 区块已加入 DeepSeek、Qwen、智谱、小米 MiMo、Kimi、自定义接口的获取说明；App 配置页同步提示用户前往官网查看。 |
| P2 | API 配置成功后的变化不明显。 | 成功后显示动态电池充电动画；下方提供“更换其他接口”按钮，点击后回到当前配置界面。 |
| P2 | 底部导航切换缺少触觉反馈。 | 已在底部 Tab 切换时加入轻量震动反馈，提升互动感。 |
