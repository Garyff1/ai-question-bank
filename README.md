...
- **图表乱码修复（v2.7.0 新增）**：化学只生成 smart_content_viewer 支持的 8 种分子（H2O/CO2/NH3/CH4/O2/N2/HCl/NaCl），物理图按关键词选择合理的 diagram_type 与 params，函数表达式清理非法字符并限长，杜绝乱码
- **音频题启用开关（v2.7.0 新增）**：出题窗口与试卷出题面板均新增绿色"启用音频题（英语听力）"开关，偏好持久化 key `enable_listening_v1`
- **出题筛选**：题型多选、目标群体、题量 1-20、启用图表开关
- **试卷生成**：按中考/高考/期中/期末/周测模板生成，含题量模板与自定义；v2.7.0 起新增图表开关
- **错题本**：按资料分组、30 天自动清理、批量删除、检索；v2.7.0 起错题卡片渲染 rich_content 图表
- **练习历史**：详情页含动画饼图（正确率）和直方图（题型分布、知识点错误）
- **收纳折叠（v2.7.0 新增）**：我的窗口历史记录折叠为最近 5 次，错题窗口折叠为最近 3 份资料×3 道题，试卷窗口折叠为最近 5 份资料，搜索模式下自动展开
...
| 版本 | 主要内容 |
| --- | --- |
| v2.7.0 | 图表乱码修复、试卷出题加图表开关、音频题启用开关（出题+试卷）、错题窗口呈现图表、收纳折叠（历史5/错题3×3/试卷5）、关于介绍更新 |
